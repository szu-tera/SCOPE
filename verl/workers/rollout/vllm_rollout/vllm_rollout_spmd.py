...
from verl.trainer.ppo.myutils import split_steps, calculate_avg_step_conf
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        # For step operation, we need to use the tokenizer.
        ...
        with self.update_sampling_params(**kwargs):
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    # compute the average step confidence
                    split_token = self.tokenizer.tokenize("\n\n")[0]
                    steps = split_steps(response_ids, split_token, self.tokenizer)
                    avg_step_conf = calculate_avg_step_conf(steps, output.outputs[sample_id].logprobs)
                    avg_step_confs.append(avg_step_conf)
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                            curr_log_prob.append(logprob[response_ids[i]].logprob)
                        rollout_log_probs.append(curr_log_prob)
            # All data in non_tensor_batch should be np.ndarray
            avg_step_confs = np.array(avg_step_confs) # FIX: Add this line to convert avg_step_confs to np.ndarray
            non_tensor_batch["confs"] = avg_step_confs
