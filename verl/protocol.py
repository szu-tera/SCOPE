...
class DataProto:
    ...
    def repeat(self, repeat_times=2, interleave=True, deepcopy=False):
        """
        ...
        Args:
            ...
            deepcopy (bool): Whether to perform deep copy of the data.
        ...
        if self.batch is not None:
            ...
            # Apply deep copy to tensors if requested
            if deepcopy:
                repeated_tensors = {key: tensor.clone() for key, tensor in repeated_tensors.items()}
        ...
        # Helpers for deep-copy behavior on non-tensor arrays
        def _repeat_deep_object(a, repeats):
            # Per-element consecutive repeats: [x1,x1,..., x2,x2,...]
            return np.array([copy.deepcopy(x) for x in a for _ in range(repeats)], dtype=object)

        def _tile_deep_object(a, repeats):
            # Whole-batch tiling: [x1,x2,..., x1,x2,...]
            return np.array([copy.deepcopy(x) for _ in range(repeats) for x in a], dtype=object)

        for key, val in self.non_tensor_batch.items():
            if deepcopy:
                # Preserve numeric arrays' shapes; deep-copy object elements
                if hasattr(val, "dtype") and val.dtype != object:
                    if interleave:
                        repeated_non_tensor_batch[key] = np.repeat(val, repeat_times, axis=0).copy()
                    else:
                    ...
                    if interleave:
                        repeated_non_tensor_batch[key] = _repeat_deep_object(val, repeat_times)
                    else:
                        repeated_non_tensor_batch[key] = _tile_deep_object(val, repeat_times)
            else:
                if interleave:
        ...
        # Apply deep copy to non-tensor data if requested
        if deepcopy:
            repeated_non_tensor_batch = copy.deepcopy(repeated_non_tensor_batch)

        # Apply deep copy to meta_info if requested
        meta_info = copy.deepcopy(self.meta_info) if deepcopy else self.meta_info
