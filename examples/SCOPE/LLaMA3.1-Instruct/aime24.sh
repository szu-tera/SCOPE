...
echo "n gpus per node: $N_GPUS_PER_NODE"
...
N_VOTES_PER_SUBGROUP=32
GROUP_SIZE=32
MINI_BATCH_SIZE=1
MICRO_BATCH_SIZE=2
LAM=0.7

DATA_LOCAL_DIR="path/to/data"
BACKBONE_PATH="path/to/${BACKBONE}"

MODEL="${TASK}-${BACKBONE}"
EXPERIMENT="SCOPE-Len@${K}k"

WANDB_PROJECT="SCOPE-verl"
...
--config-name='ppo_trainer_ttrl.yaml'\
  ...
  scope.n_votes_per_subgroup=$N_VOTES_PER_SUBGROUP \
  scope.lam=$LAM \
